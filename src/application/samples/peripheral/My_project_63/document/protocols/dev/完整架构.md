...
|------|------|-----------|----------|--------|------|------|
| WS63 ↔ ESP32-S3 | UART1 | TX→RX (GPIO15), RX←TX (GPIO16) | GPIO17(TX), GPIO18(RX) + GPIO2 (RTC唤醒) | 115200 | `\n` | 3.3V TTL |
...
│
├── main/
│   ├── main.c                        # 应用入口，状态机，FreeRTOS 任务
│   ├── main.h                        # 全局变量声明，状态枚举，结构体定义
│   │
│   └── modules/
│       ├── camera/
│       │   └── camera_module.c/.h    # OV5640 摄像头驱动
│       │
│       ├── ai/
│       │   ├── ai_module.c/.h        # AI 推理编排（三视图流程控制）
│       │   ├── mobilenet_wrapper.cpp/.h  # MobileNetV2 模型封装 (esp-dl)
│       │   ├── feature_processor.c/.h    # 多帧融合 (3帧/视图) + 特征提取
│       │   ├── similarity_matcher.c/.h   # 混合相似度计算 (70%余弦 + 30%欧几里得)
│       │   └── blur_detection.c/.h       # 拉普拉斯方差模糊检测 (阈值80)
│       │
│       ├── system/
│       │   ├── protocol_handler.c/.h     # WS63 UART1 JSON 协议处理 (~2300行)
│       │   ├── cmd_handler.c/.h          # UART0/CLI 命令处理
│       │   ├── asset_manager.c/.h        # SD 卡资产读写 (特征向量 + JPEG)
│       │   ├── storage_module.c/.h       # 存储抽象层
│       │   ├── tag_id_validator.c/.h     # Tag ID 格式验证
│       │   ├── verify_handler.c/.h       # 验证式更新逻辑
│       │   ├── verify_config.h           # 验证阈值参数
│       │   └── led_indicator.c/.h        # WS2812 RGB LED 控制
│       │
│       └── 4g/
│           ├── l610_config.h             # L610 引脚/波特率/MQTT 配置
│           ├── l610_driver.c/.h          # L610 UART 驱动，AT 指令
│           ├── l610_manager.c/.h         # L610 MQTT 管理器，心跳
│           └── l610_mqtt.c/.h            # MQTT 原语连接/发布/断开
│
├── components/                           # 供应商/本地组件 (git submodule)
│   ├── esp-dl/                           # Espressif 深度学习库
│   ├── esp-who/                          # Espressif AI 解决方案框架
│   └── esp32-camera/                     # Espressif 摄像头传感器驱动
│
├── managed_components/                   # ESP-IDF 组件注册表管理
│   ├── espressif__dl_fft/                # FFT 库
│   ├── espressif__esp-dsp/               # 数字信号处理库
│   ├── espressif__esp_jpeg/              # 软件 JPEG 编解码器
│   └── espressif__esp_new_jpeg/          # 硬件加速 JPEG 编解码器
│
└── docs/
    ├── README.md
    ...
    ├── MONITOR_CODE.md                   # 淘晶驰 T1 端侧代码完整汇总
    ├── L610_DEBUG_GUIDE.md               # L610 4G 调试指南
    ├── PROTOCOL/
    │   ├── END_TO_END_PROTOCOL.md        # 端到端协议汇总（入口文档）
    │   ├── ESP32_WS63_PROTOCOL.md        # WS63↔ESP32 JSON 协议 (v3.3)
    │   └── WS63_MONITOR_PROTOCOL.md      # WS63↔串口屏 CSV 协议 (v2.3)
    └── archive/                          # 历史文档归档
```
...
- ESP32 将实时拍摄的三视图特征与 TF 卡中已存储特征做混合相似度比对
...
| AT 指令超时 | 5 秒 | 协议规范 |
| AT 重试次数 | 3 次 | 协议规范 |
| 相似度阈值 | 0.75 (70%余弦 + 30%欧几里得) | 协议规范 |
| 模糊度阈值 | 80 分 (拉普拉斯方差) | 协议规范 |
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `main/main.c` | ~1000+ | 应用入口 + 状态机 + 任务调度 |
| `main/modules/system/protocol_handler.c` | ~2300 | WS63 JSON 协议处理 |
| `main/modules/camera/camera_module.c` | ~300 | OV5640 驱动 |
| `main/modules/ai/ai_module.c` | ~500 | AI 推理编排 |
| `main/modules/ai/mobilenet_wrapper.cpp` | ~200 | MobileNetV2 封装 |
| `main/modules/ai/feature_processor.c` | ~300 | 多帧融合 + 特征提取 |
| `main/modules/ai/similarity_matcher.c` | ~200 | 混合相似度计算 |
| `main/modules/system/asset_manager.c` | ~500 | SD 卡资产管理 |
| `main/modules/system/verify_handler.c` | ~300 | 验证式更新逻辑 |
| `main/modules/4g/l610_manager.c` | ~400 | L610 MQTT 管理 |
...
| 端到端协议汇总 | `CAM_AI/docs/PROTOCOL/END_TO_END_PROTOCOL.md` |
| WS63↔ESP32 JSON 协议 | `CAM_AI/docs/PROTOCOL/ESP32_WS63_PROTOCOL.md` |
| WS63↔串口屏 CSV 协议 | `CAM_AI/docs/PROTOCOL/WS63_MONITOR_PROTOCOL.md` |
