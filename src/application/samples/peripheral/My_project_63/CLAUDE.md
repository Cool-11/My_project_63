...
| 参数 | BS21E 值 | 说明 |
|------|----------|------|
| 广播间隔 | `0xC8` (200) | 200 × 0.625ms = **125ms**，即每秒 **8 次**广播 |
| 单标签回调频率 | ~8 次/s | 每 125ms 触发一次 `my63_seek_result_cb` |
| 32 标签总回调频率 | ~256 次/s | 这是 NMI 崩溃的直接原因 |

队列深度 256 条可缓冲约 1 秒的突发数据。出队处理速度需 > 入队速度，否则队列溢出。
...
| adv_field status 字段 | 出库（OUTSTOCK） | BS21E 广播中的标签状态 |

**BS21E 端 status 定义（广播字段）：**

| 值 | BS21E 含义 | 触发条件 |
|----|-----------|---------|
| 0x00 | NORMAL（正常） | 默认 / 寻物恢复 |
| 0x01 | FINDING（寻物中） | 收到 0x01 寻物命令 |
| 0x02 | OUTSTOCK（出库） | qty 被设为 0 |
...
BS21E status → WS63 上云 status:
  0x00 (NORMAL)  → 2 (在线，正常运行)
  0x01 (FINDING) → 2 (在线，寻物中)
  0x02 (OUTSTOCK)→ 2 (在线，出库状态)
  未扫描到       → 3 (离线)
  未注册         → 0 (空闲)
```
...
| item | string | 货物名称 |
| qty | integer | 库存数量 |
| status | integer | 0=空闲, 1=已绑定, 2=在线(NORMAL/FINDING/OUTSTOCK), 3=离线。**需从 BS21E 原始值映射，见 §9** |
